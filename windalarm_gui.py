...
while running:
    ...
    if((interval > refresh_every_sec) | firsttime):
        firsttime=False
        timezero+=interval
        lines=get_weather()
        wind  = lines.split('\n')[8]
        ...
        if value>25:
	        ...
	        lbl = Label(window, text="{0}   -   WIND IS FINE   ".format(wind), fg="green", bg=None)
